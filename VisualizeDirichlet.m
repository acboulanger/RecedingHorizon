function VisualizeDirichlet(filenameIn,  filenameMovie)
    ...
    i = 1;
    j = 1;
    while (i < experiment.args.nmaxrh+1)
          i
        discr = experiment.args.matrices.Obs*(y(i,:)'-yobs(i,:)');
        ymyd = experiment.args.matrices.trialT\discr;
        l2norm(j) = sqrt(ymyd'*(experiment.args.matrices.A*ymyd));
        j = j+1;
        i = i+10;
    end   
    figure(2)
     plot(1:size(l2norm,2),l2norm);
    
    x = experiment.args.chebyGL;
    maxy = max(max(y));
    miny = min(min(y));
    maxu = max(max(u));
    minu = min(min(u));
